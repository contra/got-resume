...
module.exports = (transfer) => {
  ...
  const stream = got.stream(transfer.url, {
    ...transfer.gotOptions,
    http2: true, // this can be removed in a later version of got when they change the default to true
    agent
  })
  ...
  stream.once('response', (res) => {
    ...
    try {
      ...
      const etag = headers.etag && headers.etag.replace(/^W\//, '')
      ...
      if (first) {
        ...
        if (transfer.etag && etag != transfer.etag) {
          throw new errors.TransferError(
