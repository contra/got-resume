...
Transfer.prototype.get = function () {
  ...
  if (!this.supportsRanges && this.position) {
    // TODO: add support for skipping entry bytes to keep going
    return this.failed(
      new errors.TransferError(
        `Transfer stopped before end - server does not support range requests to continue.`
      ),
      null,
      false
    )
  ...
  const outStream = pipeline(requestStream, transformStream, (err) => {
    this.log('Pipeline stream ended', { err })
