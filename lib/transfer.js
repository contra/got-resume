...
function Transfer(options) {
	...
	this.gotOptions = Object.assign({}, options.got, { responseType: 'buffer' });
...
Transfer.prototype.start = function () {
	...
	this.prePromise
		.then(() => {
			...
			if (!this.url)
				throw new errors.PreError(
					'pre function did not set transfer.url'
				);
			if (this.cancelled)
				throw new errors.CancelError('Transfer cancelled');
...
Transfer.prototype.get = function () {
	...
	stream.on('response', (res) => {
		this.log('Received HTTP response', {
			url: res.url,
			headers: res.headers,
		});
		...
		try {
			...
			if (this.position) {
				...
				const range = headers['content-range'].match(
					/^bytes (\d+)-\d+\/(\d+)$/
				);
				if (!range)
			...
			if (first) {
				...
				if (this.length != null)
					this.total = this.length - this.options.offset;

				// Emit progress event
				this.stream.emit('progress', {
					transferred: 0,
					total: this.total,
				});
			} else {
				...
				if (this.etag && headers.etag != this.etag)
					throw new errors.TransferError(
						`ETag has changed: '${headers.etag}' from '${this.etag}'`
					);
			}
		} catch (err) {
...
Transfer.prototype.cancel = function () {
	...
	if (this.prePromise) {
		...
		if (typeof this.prePromise.cancel == 'function')
			this.prePromise.cancel();
	} else if (this.req) {
