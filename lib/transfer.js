...
Transfer.prototype.get = function() {
	...
	transformStream.on('end', () => {
		...
		if (this.position == this.length || (this.length == null && this.res && !this.err)) { // jshint ignore:line
			...
			transfer.failed(new errors.TransferError(`Transfer stopped before end - at ${transfer.position} not ${transfer.length}`), this.err, !received);
		}
	});
...
Transfer.prototype.failed = function(err, original, empty) {
	...
	// retrying a request in the 400s is pointless
	if (original && original.res && original.res.statusCode > 400 && original.res.statusCode < 502) {
		this.fatal();
		return;
	}
